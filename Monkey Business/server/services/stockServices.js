...
export async function getOneStock (req, res) {
  ...
  queryMongoDatabase(async db => {
    const data = await db.collection('StocksTempAPI').findOne({ name: stockName }, { projection: { _id: 0 } })
    if ((await db.collection('StocksTempAPI').countDocuments({})) === 0) {
      res.status(401).json({ error: true, message: 'No Stocks in Database' })
    }
    if (data === null) {
      res.status(402).json({ error: true, message: 'No Stocks Found' })
    }
    res.json(data)
...
// TODO: get data from actual API to work on Search Page
...
function parseStockData (stockData) {
  ...
  if (stockData === undefined) {
    console.error('Error: stockData is undefined')
